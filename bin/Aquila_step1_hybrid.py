...
#### remove barcode "0_0_0" for stLFR
def remove_barcode000(h5_file_sorted,h5_dir,chr_num): 
    temp_file = h5_dir + "temp_chr" + str(chr_num)
    Popen("mv " + h5_file_sorted + " " + temp_file,shell=True).wait()
    fw = open(h5_file_sorted,"w")
    with open(temp_file,"r") as f:
        for line in f:
            ...
            if barcode != "0_0_0":
                fw.write(line)
    fw.close()
    Popen("rm " + temp_file,shell=True).wait()
    ...
    if len(sys.argv) == 1:
        ...
        for _num in range(len(bam_list)):
            ...
            for chr_num in range(chr_start, chr_end + 1):
                ...
                remove_barcode000(h5_dir + sample_name + "_chr" + str(chr_num) + "_sorted.h5",h5_dir,chr_num)   ### add for remove barcode "0_0_0" for stLFR
